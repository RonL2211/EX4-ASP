...
namespace Matala2_ASP.Controllers
{
    ...
    public class MovieController : ControllerBase
    {
        ...
        {
            bool success = Movie.Insert(newMovie);
       ...
       // GET api/Movie/wishlist/{id}
        ...
        //[HttpGet("user/{userId}")]
        //public ActionResult GetAllUserMovies(int userId)
        //{
        //    List<Movie> movies = Movie.GetAllUserMovies(userId); // Implement `GetMoviesForUser` in `Movie.cs`
        //    if (movies == null || movies.Count == 0) return NotFound("No movies found for this user.");
        //    return Ok(movies);
        //}

        // DELETE: api/Movie/unlink/{userId}/{movieId}
        [HttpDelete("wishlist/{userId}/{movieId}")]
